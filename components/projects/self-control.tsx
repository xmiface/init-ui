...
const Index = () => {
  ...
  const pageNameRef = useRef<null | HTMLInputElement>(null);
  ...
  useEffect(() => {
    axios
      .post('/api/projects/self-control/create-page', { type: 'getFolders' })
  ...
  const hCreateProject = () => {
    if (pageNameRef?.current) {
      ...
      axios
        .post('/api/projects/self-control/create-page', { type: 'createPage', folder, title })
        .catch((err) => console.log(err))
    ...
    <div className=" bg-[#0E0E0E] h-screen px-4 w-full text-white">
      <div className="w-[1440px] mx-auto text-xl">
        ...
        <div className="flex gap-8 h-12 my-8">
          <input
            className="bg-slate-900 rounded-xl border-twitchdarkpink hover:border-twitchpink duration-300 py-2 px-4"
            type="text"
            placeholder="page name"
            ref={pageNameRef}
            onChange={handleType}
          />
          <button onClick={hCreateProject} className={sButton}>
            create page
          </button>
        ...
        <div className="flex gap-8">
          <div className="flex flex-col gap-8 w-[300px]">
            ...
            {folders.map((el) => (
              <button
                key={el}
                className={clsx(sButton, el === folder ? 'text-twitchpink' : '')}
                onClick={() => {
                  setFolder(el);
                  axios
                    .post('/api/projects/self-control/create-page', { type: 'getSubFolders', folder: el })
                    .catch((err) => console.log(err))
          ...
          <div className="flex flex-col gap-8 w-[300px]">
            <h2>sub folders</h2>
            {subFolders.map((el) => (
              <button key={el} className={sButton}>
                {el}
              </button>
            ))}
          </div>
