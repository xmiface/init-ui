...
const sSpinButton = ' bg-[rgba(0,0,0,0.36)] disabled:bg-[rgba(73,71,71,0.36)] absolute bottom-[-150px] left-[320px] w-[320px] h-16';
...
const createTopImages = (values: number[][], images: string[], root: Root) => {
  values.forEach((value, col) => {
    ...
    value.forEach((item, row) => {
      ...
      div.style.height = 600 / value.length + 'px';
      div.style.margin = '0 auto';
      img.src = images ? images[blockValue] : '';
...
const Main: FC<{ setFullPageBg: (value: string) => void }> = ({ setFullPageBg }) => {
  ...
  return (
    <div className="left-0 top-0 h-full w-full rounded-3xl overflow-hidden">
      <button ref={spaceRef} onClick={handleClick} autoFocus className={sSpinButton}>
        SPIN
      </button>
